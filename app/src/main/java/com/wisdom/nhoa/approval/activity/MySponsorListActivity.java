...
public class MySponsorListActivity extends BaseActivity {
    ...
    public void initViews() {
        getData(pull);
        setTitle(R.string.approval_list_title);
    ...
    private void getData(final PullToRefreshLayout pullToRefreshLayout) {
        ...
        HttpUtil.httpGet(ConstantUrl.GET_MY_APPROVAL_LIST_URL, httpParams, new JsonCallback<BaseModel<List<ApprovalListModel>>>() {
            @Override
            ...
            public void onSuccess(final BaseModel<List<ApprovalListModel>> listBaseModel, Call call, Response response) {
               U.closeLoadingDialog();
                if (listBaseModel.results.size() > 0) {
                    puallableListView.setAdapter(new MySponsorListAdapter(MySponsorListActivity.this, listBaseModel.results));
                    //子项点击事件，跳转到详情页面
                    puallableListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                        ...
                        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                            ...
                            Intent intent = new Intent(MySponsorListActivity.this, MySponsorDetailActivity.class);
