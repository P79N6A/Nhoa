...
import com.wisdom.nhoa.approval.adapter.PopWheelPeopleNameAdapter;
import com.wisdom.nhoa.approval.adapter.PopWheelPeopleTreeAdapter;
 ...
 * @class describe：提交审批的时候选择审批人的滚轮弹出
 * @time 2018/7/13 17:13
 * @change
 */
public class PeopleTreeWindowWheel extends PopupWindow {
    Context mContext;
    ...
    public PeopleTreeWindowWheel(Context context) {
        super(context);
...
//        FIXME 设置进出动画
        ...
        HttpUtil.httpGet(ConstantUrl.GET_PEOPLE_TREE_URL, httpParams, new JsonCallback<BaseModel<List<OrganizationModel>>>() {
            ...
            public void onSuccess(final BaseModel<List<OrganizationModel>> listBaseModel, Call call, Response response) {
                ...
                if (listBaseModel.results.size() > 0) {
                    final PopWheelPeopleTreeAdapter popWheelPeopleTreeAdapter = new PopWheelPeopleTreeAdapter(mContext, listBaseModel.results);
                    lvStuffSubject.setAdapter(popWheelPeopleTreeAdapter);
                    lvStuffSubject.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                        @Override
                        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                            popWheelPeopleTreeAdapter.setSelectedPosition(position);
                            popWheelPeopleTreeAdapter.notifyDataSetInvalidated();
                            final List<OrganizationModel.ChiledBean> listChildBean = listBaseModel.results.get(position).getChiled();
                            lvStuffName.setAdapter(new PopWheelPeopleNameAdapter(mContext, listChildBean));
                            lvStuffName.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                                ...
                                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                                    if (listener != null) {
                                        ...
                                        PeopleTreeWindowWheel.this.dismiss();
                                    }
                                }
                            });
                        }
    ...
    public interface onChildItemClikedListener {
        void onChildItemClicked(OrganizationModel.ChiledBean childBean);
    }
}
