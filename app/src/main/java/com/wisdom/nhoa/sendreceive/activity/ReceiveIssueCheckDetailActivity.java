...
public class ReceiveIssueCheckDetailActivity extends BaseActivity implements RadioGroup.OnCheckedChangeListener, ScrollViewWithScrollListener.ScrollViewListener, ViewPager.OnPageChangeListener {
    ...
    public void initViews() {
        setTitle(R.string.sendreceive_receive_check);
    ...
    private void getToDoDetails() {
        ...
        HttpUtil.httpGet(ConstantUrl.GET_COMPLETE_ISSUE_DETAILS, params, new JsonCallback<BaseModel<ToDoModel>>() {
            ...
            public void onSuccess(BaseModel<ToDoModel> listBaseModel, Call call, Response response) {
                ...
                BroadCastManager.getInstance().sendBroadCast(ReceiveIssueCheckDetailActivity.this, intent);
...
//            上传文件到服务器的逻辑
            HttpUtil.uploadFiles(ConstantUrl.FILEHANDINGSHEET_SIGN, params1, new StringCallback() {
                ...
                public void onError(Call call, Response response, Exception e) {
                    super.onError(call, response, e);
                    Toast.makeText(ReceiveIssueCheckDetailActivity.this, "上传失败", Toast.LENGTH_SHORT).show();
                    Log.e(TAG, "onError: " + e);
                ...
                public void onSuccess(String s, Call call, Response response) {
                    ...
                    try {
                        ...
                        if ("0".equals(code)) {
                            Toast.makeText(ReceiveIssueCheckDetailActivity.this, "上传成功", Toast.LENGTH_SHORT).show();
                            // 签名成功后的处理
                            Intent intentRefresh = new Intent();
                            intentRefresh.setAction(ConstantString.BROADCAST_REFRESH_TAG);
                            BroadCastManager.getInstance().sendBroadCast(ReceiveIssueCheckDetailActivity.this, intentRefresh);
                        } else {
                            Toast.makeText(ReceiveIssueCheckDetailActivity.this, jsonObject.getString("error_msg"), Toast.LENGTH_SHORT).show();
                        }
                        if (mWritePadDialog != null) {
                            mWritePadDialog.dismiss();
                        }
