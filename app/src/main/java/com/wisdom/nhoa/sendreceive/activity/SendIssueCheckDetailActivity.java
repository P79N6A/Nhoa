...
/**
 * @author lxd
 * @ProjectName project：发文查阅功能
 * @class package：
 * @class describe：SendIssueCheckDetailActivity
 * @time 13:40
 * @change
 */
public class SendIssueCheckDetailActivity extends BaseActivity implements RadioGroup.OnCheckedChangeListener, ScrollViewWithScrollListener.ScrollViewListener, ViewPager.OnPageChangeListener {

    public static final String TAG = ReceiveIssueCheckDetailActivity.class.getSimpleName();
    ...
    public void initViews() {
        setTitle(R.string.approval_issue_check);
...
//                approvalSv.scrollTo(0, 0);
    ...
    private void getToDoDetails() {
        ...
        HttpUtil.httpGet(ConstantUrl.GET_COMPLETE_ISSUE_DETAILS, params, new JsonCallback<BaseModel<ToDoModel>>() {
            @Override
            public void onSuccess(BaseModel<ToDoModel> listBaseModel, Call call, Response response) {
                ...
                BroadCastManager.getInstance().sendBroadCast(SendIssueCheckDetailActivity.this, intent);
...
//            上传文件到服务器的逻辑
            HttpUtil.uploadFiles(ConstantUrl.FILEHANDINGSHEET_SIGN, params1, new StringCallback() {
                ...
                public void onError(Call call, Response response, Exception e) {
                    super.onError(call, response, e);
                    Toast.makeText(SendIssueCheckDetailActivity.this, "上传失败", Toast.LENGTH_SHORT).show();
                    Log.e(TAG, "onError: " + e);
                ...
                public void onSuccess(String s, Call call, Response response) {
                    ...
                    try {
                        ...
                        if ("0".equals(code)) {
                            Toast.makeText(SendIssueCheckDetailActivity.this, "上传成功", Toast.LENGTH_SHORT).show();
                            // 签名成功后的处理
                            Intent intentRefresh = new Intent();
                            intentRefresh.setAction(ConstantString.BROADCAST_REFRESH_TAG);
                            BroadCastManager.getInstance().sendBroadCast(SendIssueCheckDetailActivity.this, intentRefresh);
                        } else {
                            Toast.makeText(SendIssueCheckDetailActivity.this, jsonObject.getString("error_msg"), Toast.LENGTH_SHORT).show();
                        }
                        if (mWritePadDialog != null) {
                            mWritePadDialog.dismiss();
                        }
