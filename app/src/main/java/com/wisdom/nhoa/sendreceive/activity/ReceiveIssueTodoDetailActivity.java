...
public class ReceiveIssueTodoDetailActivity extends BaseActivity implements RadioGroup.OnCheckedChangeListener, ScrollViewWithScrollListener.ScrollViewListener, ViewPager.OnPageChangeListener {
    ...
    private ReceiveIssueTodoDetailActivity.MyReceiver finishreceiver;//关闭activity广播
    ...
    public void initViews() {
        ...
        //接收广播关闭页面
        finishreceiver = new ReceiveIssueTodoDetailActivity.MyReceiver();
...
//                approvalSv.scrollTo(0, 0);
    ...
    private void getToDoDetails() {
        ...
        HttpUtil.httpGet(ConstantUrl.BACKLOGDETAILS, params, new JsonCallback<BaseModel<ToDoModel>>() {
            ...
            public void onSuccess(BaseModel<ToDoModel> listBaseModel, Call call, Response response) {
                ...
                BroadCastManager.getInstance().sendBroadCast(ReceiveIssueTodoDetailActivity.this, intent);
                Log.i(TAG, "是否有办结状态: "+listBaseModel.results.getNodeType());
                if (ConstantString.FILE_TYPE_COMPLETE.equals(listBaseModel.results.getNodeType())) {
                    hasComplete = true;
                }
    ...
    private void send() {
        ...
        intent.putExtra("doctype", ConstantString.SEND_RECEIVE_DOC_TYPE_RECEIVE);
