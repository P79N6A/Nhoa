...
public class SendIssueSponsorDetailActivity extends BaseActivity implements RadioGroup.OnCheckedChangeListener, ScrollViewWithScrollListener.ScrollViewListener, ViewPager.OnPageChangeListener {
    ...
    private void getToDoDetails() {
        ...
        HttpUtil.httpGet(ConstantUrl.BACKLOGDETAILS, params, new JsonCallback<BaseModel<ToDoModel>>() {
            ...
            public void onSuccess(BaseModel<ToDoModel> listBaseModel, Call call, Response response) {
                ...
                BroadCastManager.getInstance().sendBroadCast(SendIssueSponsorDetailActivity.this, intent);
...
//            上传文件到服务器的逻辑
            HttpUtil.uploadFiles(ConstantUrl.FILEHANDINGSHEET_SIGN, params1, new StringCallback() {
                ...
                public void onError(Call call, Response response, Exception e) {
                    super.onError(call, response, e);
                    Toast.makeText(SendIssueSponsorDetailActivity.this, "上传失败", Toast.LENGTH_SHORT).show();
                    Log.e(TAG, "onError: " + e);
                ...
                public void onSuccess(String s, Call call, Response response) {
                    ...
                    try {
                        ...
                        if ("0".equals(code)) {
                            Toast.makeText(SendIssueSponsorDetailActivity.this, "上传成功", Toast.LENGTH_SHORT).show();
                            // 签名成功后的处理
                            Intent intentRefresh = new Intent();
                            intentRefresh.setAction(ConstantString.BROADCAST_REFRESH_TAG);
                            BroadCastManager.getInstance().sendBroadCast(SendIssueSponsorDetailActivity.this, intentRefresh);
                        } else {
                            Toast.makeText(SendIssueSponsorDetailActivity.this, jsonObject.getString("error_msg"), Toast.LENGTH_SHORT).show();
                        }
                        if (mWritePadDialog != null) {
                            mWritePadDialog.dismiss();
                        }
