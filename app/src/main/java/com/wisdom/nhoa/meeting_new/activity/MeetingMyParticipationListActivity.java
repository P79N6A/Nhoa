...
import com.wisdom.nhoa.meeting_new.adapter.MeetingMyParticipationAdapter;
...
/**
 * 类 {类名称}  {此类功能描述}会议 --我审核的
 *
 * @author lxd
 * @date 2018-10-24
 * 节日快乐
 */
public class MeetingMyParticipationListActivity extends BaseActivity implements MeetingMyParticipationAdapter.OnChildItemClickedListener{
    ...
    TextView tv_no_data;
    MeetingMyParticipationAdapter adapter;

    @Override
    public void setlayoutIds() {
        setContentView(R.layout.activity_meeting_my_participation_list);
    }
    ...
    private void getData() {
        ...
        HttpUtil.httpGet(ConstantUrl.MEETING_MY_PARTICIPATION_LIST_URL, params, new JsonCallback<BaseModel<List<MeetingMySponsorModel>>>() {
            @Override
            public void onSuccess(BaseModel<List<MeetingMySponsorModel>> model, Call call, Response response) {
                ...
                if (model.results.size() > 0) {
                    ...
                    adapter = new MeetingMyParticipationAdapter(model.results, MeetingMyParticipationListActivity.this);
                    adapter.setListener(MeetingMyParticipationListActivity.this);
